...
// Estrutura para um nó da árvore binária não balanceada
struct no {
    int chave;
    struct no* esquerdo;
    struct no* direito;
...
struct AVLno* insereAVL(struct AVLno* no, int chave) {
    if (no == NULL) return noAVLno(chave);
...
int main() {
    ...
    start = clock();
    for (int i = 0; i < 20000; i++) { // Repetir 20000 vezes para tamanho maximo to arquivo de entrada, para testes diminuir o loop
